    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6">
      <div className="max-w-md w-full space-y-4 rounded-xl border border-slate-800 bg-slate-900 p-6 shadow-lg">
        ...
        <p className="text-sm text-slate-400">
          Phase 0 scaffold — this page proves the web client can reach the Go
          API at <code className="text-slate-300">{API_BASE_URL}</code>.
        </p>
        ...
        <div className="rounded-lg bg-slate-800/60 p-4 text-sm">
          ...
          {data && (
            <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1">
              ...
              <dd className={data.db === 'ok' ? 'text-emerald-400' : 'text-red-400'}>
                {data.db}
              </dd>
              {data.error && (
                <>
